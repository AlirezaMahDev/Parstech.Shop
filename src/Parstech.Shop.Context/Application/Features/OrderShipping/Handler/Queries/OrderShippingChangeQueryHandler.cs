...
public class OrderShippingChangeQueryHandler : IRequestHandler<OrderShippingChangeQueryReq, long>
{
    ...
    public OrderShippingChangeQueryHandler(IMapper mapper, IMediator mediator,
    ...
    public async Task<long> Handle(OrderShippingChangeQueryReq request, CancellationToken cancellationToken)
    {
        ...
        var CurrentOrderShipping = await _orderShippingRepository.GetOrderShippingByOrderId(request.OrderId);
        var shippingType = 0;
        if (request.Type == "Refresh")
        {
            ...
            {
                shippingType = 4;
                var shippingTypeItem = await _shippingTypeRep.GetAsync(shippingType);
            ...
            else
            {
                ...
                {
                    var userShipping = await _userShippingRep.GetAsync(int.Parse(CurrentOrderShipping.UserShippingId));
                    ...
                    var shippingTypeItem = await _shippingTypeRep.GetAsync(CurrentOrderShipping.ShippingTypeId);
                    return shippingTypeItem.Price;
                }
                else
                {
        ...
        else
        {
            ...
            var orderShipping = new Parstech.Shop.Context.Domain.Models.OrderShipping();


            if (request.UserShippingId != 0)
            {
                var userShipping = await _userShippingRep.GetAsync(request.UserShippingId);
                if (userShipping.State == "تهران")
                {
                    shippingType = 1;
                }
