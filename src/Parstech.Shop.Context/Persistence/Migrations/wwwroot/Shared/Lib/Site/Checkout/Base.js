...
function FillData(response) {
    ...
    DataSection.insertAdjacentHTML("beforeend", "<div class='col-lg-8 order-lg-1 rtl'><div class='container bg-gray-7  mb-md-0 border-width-2 border-color-1 productImageBackRadius'><div class='mb-10 cart-table productImage'><table class='table' cellspacing='0'><thead><tr><th class='product-remove'>&nbsp;</th><th class='product-thumbnail'>&nbsp;</th><th class='product-name'>محصول</th><th class='product-price'>قیمت</th><th class='product-quantity w-lg-20'>تعداد</th><th class='product-subtotal w-lg-15'>تخفیف</th><th class='product-subtotal w-lg-15'>جمع قیمت <br/> <small class='Green font-weight-bold font-size-11'>با احتساب مالیات و تخفیف</samll></th></tr></thead><tbody id='DetailSection'></tbody></table></div></div></div>");
    var DetailSection = document.getElementById("DetailSection");
    Details.forEach(function (element) {
        var count = element.count;
        DetailSection.insertAdjacentHTML("beforeend", "<tr class=''><td class='text-center'><a onclick='DeleteDetail(" + element.id + ")' href='#' class='text-gray-32 font-size-26'>×</a></td><td class='d-none d-md-table-cell'><a href='#'><img class='img-fluid max-width-8 p-1 border border-color-1' src='/Shared/Images/Products/" + element.image + "' alt='ALT'></a></td><td data-title='محصول'><a href='#' class='text-gray-90  font-size-12 font-weight-bold'>" + element.productName + "</a></td><td data-title='قیمت'><span class='font-weight-bold'>" + separate(element.price) + "</span></td><td data-title='تعداد'><span class='sr-only'>Quantity</span><div class='border rounded-pill py-1 width-122 w-xl-80 px-3 border-color-1'><div class='js-quantity row align-items-center'><div class='col-auto pr-1'><small class='font-weight-bold pr-3  pl-3 border-0'>" + element.count + "</small><a onclick='ChangeCount(" + element.id + ",1000)' class='js-minus btn btn-icon btn-xs btn-outline-secondary rounded-circle border-0' href='javascript:;'><small class='fas fa-minus btn-icon__inner'></small></a><a onclick='ChangeCount(" + element.id + ",1001)' class='js-plus btn btn-icon btn-xs btn-outline-secondary rounded-circle border-0' href='javascript:;'><small class='fas fa-plus btn-icon__inner'></small></a></div></div></div></td><td data-title='تخفیف'><span class='font-weight-bold'>" + separate(element.discount) + "</span></td><td data-title='جمع قیمت'><span class='font-weight-bold'>" + separate(element.total) + " </span></td></tr >");
    });
    DetailSection.insertAdjacentHTML("beforeend", "<tr><td colspan='6' class='border-top space-top-2 justify-content-center'><div class='pt-md-3'><div class='d-block d-md-flex flex-center-between'><div class='mb-3 mb-md-0 w-xl-50'><div class='input-group'><input type='text' class='form-control' id='couponCode' placeholder='کد تخفیف خود را وارد نمایید' aria-label='Coupon code' aria-describedby='subscribeButtonExample2' required><div><button  onclick='UseCoupon(" + Order.orderId + ")' class='btn btn-Custom m-2 btn-block BgOrange White px-4' type='button' ><i class='fas fa-tags d-md-none'></i><span class='d-none d-md-inline'>اعمال تخفیف</span></button></div></div><label class='sr-only' >Coupon code</label></div></div></div></td></tr>");
    var PayTypes = response.object.payTypes;
    OrderSection.insertAdjacentHTML("beforeend", "<h6 class='font-weight-bold'><i class=' fas fa-cash-register p-2 Blue'></i>روش پرداخت</h5>");
    PayTypes.forEach(function (element) {
        if (response.object.orderPay != null) {
            var pay = response.object.orderPay;
            if (element.id == pay.id) {
                OrderSection.insertAdjacentHTML("beforeend", "<div  class='pointer border-top border-width-3 border-color-1 pt-3 mb-3'><div   id='basicsAccordion" + element.id + "'><div class='border-bottom border-color-1 border-dotted-bottom'><div class='p-3' id='basicsHeading" + element.id + "'><div onclick='SetOrderPay(" + element.id + ")' class='custom-control custom-radio'><label class='pointer form-label' for='" + element.id + "' data-toggle='collapse' data-target='#basicsCollapse" + element.id + "' aria-expanded='false' aria-controls='basicsCollapse" + element.id + "'> <i class='ml-2 ec ec-arrow-down-search'></i>" + element.typeName + "<br /> <small class='Green font-size-19 font-weight-bold'><i class=' fas fa-circle-check p-2 font-size-17'></i>انتخاب شده</small></label></div></div><div id='basicsCollapse" + element.id + "' class='collapse  border-top border-color-1 border-dotted-top bg-dark-lighter' aria-labelledby='basicsHeading" + element.id + "' data-parent='#basicsAccordion" + element.id + "'><div class='p-4'><i class=' fas fa-circle-info p-2 Blue'></i>" + element.description + "</div></div></div></div></div>");
            }
            else {
                OrderSection.insertAdjacentHTML("beforeend", "<div  class='pointer border-top border-width-3 border-color-1 pt-3 mb-3'><div   id='basicsAccordion" + element.id + "'><div class='border-bottom border-color-1 border-dotted-bottom'><div class='p-3' id='basicsHeading" + element.id + "'><div onclick='SetOrderPay(" + element.id + ")' class='custom-control custom-radio'><label class='pointer form-label' for='" + element.id + "' data-toggle='collapse' data-target='#basicsCollapse" + element.id + "' aria-expanded='false' aria-controls='basicsCollapse" + element.id + "'> <i class='ml-2 ec ec-arrow-down-search'></i>" + element.typeName + "</label></div></div><div id='basicsCollapse" + element.id + "' class='collapse  border-top border-color-1 border-dotted-top bg-dark-lighter' aria-labelledby='basicsHeading" + element.id + "' data-parent='#basicsAccordion" + element.id + "'><div class='p-4'><i class=' fas fa-circle-info p-2 Blue'></i>" + element.description + "</div></div></div></div></div>");
...
function showComplete() {
    ...
    if (Data.orderPay.id == 1 || Data.orderPay.id == 5) {
        ShowComplete2(null);
    }
    else {
        $.ajax({
