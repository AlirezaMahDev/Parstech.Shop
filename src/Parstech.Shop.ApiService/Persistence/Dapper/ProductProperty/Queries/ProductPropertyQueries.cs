...
namespace Parstech.Shop.ApiService.Persistence.Dapper.ProductProperty.Queries;

public class ProductPropertyQueries : IProductPropertyQueries
{
    public string GetCommonByTwoProductId =>
        "SELECT PropertyId FROM ProductProperty AS C WHERE C.ProductId=@firstProductId and C.PropertyId IN (SELECT  PropertyId  FROM ProductProperty AS E WHERE (E.ProductId=@secoundProductId) );";

    public string GetOneByProductIdAndPropertyId =>
    ...
    public string GeAllCompareUserProductsByUserId =>
        "SELECT* FROM UserProduct WHERE UserId=@userId AND Type='Compare'";

    public string GeAllFavoriteUserProductsByUserId =>
        "SELECT* FROM UserProduct WHERE UserId=@userId AND Type='Favorite'";
}