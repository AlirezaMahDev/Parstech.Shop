...
public class WalletRepository : GenericRepository<Wallet>, IWalletRepository
{
    ...
    public WalletRepository(DatabaseContext context, IUserBillingRepository userBillingRep, IMapper mapper) :
        base(context)
    {
        _context = context;
    ...
    {
        Wallet? wallet = await GetAsync(walletTransactionDto.WalletId);
    ...
    {
        Wallet? wallet = await _context.Wallets.SingleOrDefaultAsync(z => z.UserId == userId);
        long result = 0;
        switch (type)
        {
            case "Amount":
