...
public class WalletTransactionRepository : GenericRepository<WalletTransaction>, IWalletTransactionRepository
{
    ...
    public WalletTransactionRepository(DatabaseContext context,
    ...
    {
        if (await _context.WalletTransactions.AnyAsync(u =>
                u.WalletId == WalletId && u.Active == true && u.Type == TypeName))
        {
            return false;
        }
        else
    ...
    {
        WalletTransaction? item = await _context.WalletTransactions.FindAsync(parentId);
        return item;
    }
    ...
    public async Task<FecilitiesDto> CreateNewFesilities(FecilitiesDto request)
    {
        ...
        int month = int.Parse(std[1]);
        int day = int.Parse(std[2]);
        ...
        Random random = new();
        int tackingCode = random.Next(10000000, 99999999);
        string Description = $"شناسه تسهیلات: {request.ParentFecilitiesId} | {request.OrderCode} اقساط سفارش ";
        //var currentTransaction = new WalletTransaction();

        foreach (string item in Dates)
        {
            string[] fgd = item.Split('/');
            DateTime date = new(int.Parse(fgd[0]),
                int.Parse(fgd[1]),
                int.Parse(fgd[2]),
                new PersianCalendar()
            );
            ...
            if (Transactions.Count() > 0)
            {
                ...
                if (table != null)
                {
                    ...
                    for (int i = 0; i < Transactions.Count; i++)
                    {
                        ...
                        newRow.Cells[1]
                            .Paragraphs.First()
                            .ReplaceText("اقساط", Transactions[i].Price.ToString, false, RegexOptions.IgnoreCase);
                        newRow.Cells[2]
                            .Paragraphs.First()
                            .ReplaceText("سررسید",
                                Transactions[i].CreateDate.ToShamsi(),
                                false,
                                RegexOptions.IgnoreCase);
                        newRow.Cells[2].Paragraphs.First().Alignment = Alignment.left;
                        newRow.Cells[3]
                            .Paragraphs.First()
                            .ReplaceText("مهلت پرداخت",
                                Transactions[i].ExpireDate.Value.ToShamsi(),
                                false,
                                RegexOptions.IgnoreCase);
                        newRow.Cells[4]
                            .Paragraphs.First()
                            .ReplaceText("وضعیت", Transactions[i].TypeId.ToString, false, RegexOptions.IgnoreCase);
                        newRow.Cells[5]
                            .Paragraphs.First()
                            .ReplaceText("کد پیگیری", Transactions[i].TrackingCode, false, RegexOptions.IgnoreCase);
                    }
                }
    ...
    {
        WalletTransaction? item = await _context.WalletTransactions.OrderBy(u => u.CreateDate)
            .LastOrDefaultAsync(u => u.Description == orderCode);
        return item;
    }
    ...
    public async Task<WalletTransaction> GetActiveAghsat(int walletId, string type)
    {
        WalletTransaction? item =
            await _context.WalletTransactions.FirstOrDefaultAsync(u =>
                u.WalletId == walletId && u.Type == type && u.Active == true);
    ...
    {
        WalletTransaction? item =
            await _context.WalletTransactions.FirstOrDefaultAsync(u => u.TrackingCode == trackingCode);
        return item;
    }

    public async Task<bool> ExistTransactionForUser(int UserId, string TracsactionCode)
    {
        Wallet? wallet = await _context.Wallets.FirstOrDefaultAsync(u => u.UserId == UserId);
        if (await _context.WalletTransactions.AnyAsync(u =>
                u.WalletId == wallet.WalletId && u.TrackingCode == TracsactionCode))
        {
            return true;
        }
        else
