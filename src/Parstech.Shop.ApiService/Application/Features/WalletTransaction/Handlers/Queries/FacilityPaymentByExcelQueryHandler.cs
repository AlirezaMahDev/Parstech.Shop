    ...
    public async Task<ResponseDto> Handle(FacilityPaymentByExcelQueryReq request, CancellationToken cancellationToken)
    {
        ...
        using (ExcelPackage package = new(new FileInfo(FilePath)))
        {
            ...
            foreach (List<string> rowData in DATA)
            {
                ...
                string TransactionCode = rowData[1];
                ...
                if (!await _walletTransactionRep.ExistTransactionForUser(userId, TransactionCode))
                {
                    ErrorList error = new()
                    {
                        Caption = TransactionCode, ErrorMessage = "کد تراکتش برای کاربر یافت نشد"
                    };
                    errors.Add(error);
                }
            }
            ...
            foreach (List<string> rowData in DATA)
            {
                ...
                if (!result.IsSuccessed)
                {
                    ErrorList error = new()
                    {
                        Caption = TransactionCode, ErrorMessage = "خطایی رخ داده است.امکان تسویه تراکتش وجود ندارد"
                    };
                    errors.Add(error);
                }
            }
