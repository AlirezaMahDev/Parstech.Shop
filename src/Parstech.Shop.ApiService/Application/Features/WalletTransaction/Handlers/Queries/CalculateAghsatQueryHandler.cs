...
public class CalculateAghsatQueryHandler : IRequestHandler<CalculateAghsatQueryReq, ResponseDto>
{
    ...
    public async Task<ResponseDto> Handle(CalculateAghsatQueryReq request, CancellationToken cancellationToken)
    {
        ...
        else if (request.price > TashilatPrice)
        {
            ...
            FecilitiesDto res = _walletTransactionRep.GenerateNewFesilities(req);
            result.Object = res;
            result.IsSuccessed = true;
            result.Message =
                "اقساط قابل پرداخت سفارش شما محاسبه گردید.پس از تائید درخواست جهت تسویه و پرداخت مبلغ ما به التفاوت سفارش به درگاه انتقال داده خواهید شد ";
        }
        else
        {
            FecilitiesDto req = new();
            ...
            result.IsSuccessed = true;
            result.Message =
                "اقساط قابل پرداخت سفارش شما محاسبه گردید.لطفا با دقت آن را بررسی کرده و در صورت اطمینان سفارش خود را تائید فرمایید";
        }
