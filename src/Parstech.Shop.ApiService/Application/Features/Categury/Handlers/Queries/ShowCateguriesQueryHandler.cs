...
public class ShowCateguriesQueryHandler : IRequestHandler<ShowCateguriesQueryReq, List<ParrentCateguryShowDto>>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        List<Shared.Models.Categury> Parrents = DapperHelper.ExecuteCommand(_connectionString,
            conn => conn.Query<Shared.Models.Categury>(_categuryQueries.GetMenuParrentCateguries).ToList());

        foreach (Shared.Models.Categury parrent in Parrents)
        {
            ParrentCateguryShowDto? parrentDto = _mapper.Map<ParrentCateguryShowDto>(parrent);


            List<Shared.Models.Categury> Childs1 = DapperHelper.ExecuteCommand(_connectionString,
                conn => conn.Query<Shared.Models.Categury>(_categuryQueries.GetMenuChalidOfParrentCateguries,
                        new { @parentId = parrent.GroupId, @row = 1 })
                    .ToList());
            foreach (Shared.Models.Categury subParrent in Childs1)
            {
                SubParrentCateguryShowDto? SubParrentDto = _mapper.Map<SubParrentCateguryShowDto>(subParrent);
                SubParrentDto.Childs = new();


                List<Shared.Models.Categury> SubChilds = DapperHelper.ExecuteCommand(_connectionString,
                    conn => conn.Query<Shared.Models.Categury>(_categuryQueries.GetMenuChalidOfChildCateguries,
                            new { @parentId = subParrent.GroupId })
                        .ToList());
                parrentDto.Childs1.Add(SubParrentDto);
                foreach (Shared.Models.Categury subp in SubChilds)
                {
                    ...
                    List<Shared.Models.Categury> SubChilds2 = DapperHelper.ExecuteCommand(_connectionString,
                        conn => conn.Query<Shared.Models.Categury>(_categuryQueries.GetMenuChalidOfChildCateguries,
                                new { @parentId = subp.GroupId })
                            .ToList());


                    foreach (Shared.Models.Categury sub in SubChilds2)
                    {
            ...
            List<Shared.Models.Categury> Childs2 = DapperHelper.ExecuteCommand(_connectionString,
                conn => conn.Query<Shared.Models.Categury>(_categuryQueries.GetMenuChalidOfParrentCateguries,
                        new { @parentId = parrent.GroupId, @row = 2 })
                    .ToList());
            foreach (Shared.Models.Categury subParrent in Childs2)
            {
                SubParrentCateguryShowDto? SubParrentDto = _mapper.Map<SubParrentCateguryShowDto>(subParrent);
                SubParrentDto.Childs = new();
            ...
            List<Shared.Models.Categury> Childs3 = DapperHelper.ExecuteCommand(_connectionString,
                conn => conn.Query<Shared.Models.Categury>(_categuryQueries.GetMenuChalidOfParrentCateguries,
                        new { @parentId = parrent.GroupId, @row = 3 })
                    .ToList());
            foreach (Shared.Models.Categury subParrent in Childs3)
            {
                SubParrentCateguryShowDto? SubParrentDto = _mapper.Map<SubParrentCateguryShowDto>(subParrent);
                SubParrentDto.Childs = new();
            ...
            List<Shared.Models.Categury> Childs4 = DapperHelper.ExecuteCommand(_connectionString,
                conn => conn.Query<Shared.Models.Categury>(_categuryQueries.GetMenuChalidOfParrentCateguries,
                        new { @parentId = parrent.GroupId, @row = 4 })
                    .ToList());
            foreach (Shared.Models.Categury subParrent in Childs4)
            {
                SubParrentCateguryShowDto? SubParrentDto = _mapper.Map<SubParrentCateguryShowDto>(subParrent);
                SubParrentDto.Childs = new();
