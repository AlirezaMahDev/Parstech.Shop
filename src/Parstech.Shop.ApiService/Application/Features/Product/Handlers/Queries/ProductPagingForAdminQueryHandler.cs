...
public class ProductPagingForAdminQueryHandler : IRequestHandler<ProductPagingForAdminQueryReq, ProductPageingDto>
{
    ...
    public async Task<ProductPageingDto> Handle(ProductPagingForAdminQueryReq request,
        CancellationToken cancellationToken)
    {
        ...
        {
            categury =
                $"AND EXISTS (SELECT 1 FROM dbo.ProductCategury WHERE ProductId = dbo.Product.Id AND CateguryId = {request.ProductParameterDto.Categury}) ";
        }
        ...
        products = DapperHelper.ExecuteCommand(_connectionString, conn => conn.Query<DapperProductDto>(query).ToList());
        ...
        string AllListquery =
            $"SELECT dbo.Product.Id FROM dbo.Product where TypeId!=3 {categury} {isActive} {serach} ORDER BY CreateDate Desc";
        List<int> AllList =
