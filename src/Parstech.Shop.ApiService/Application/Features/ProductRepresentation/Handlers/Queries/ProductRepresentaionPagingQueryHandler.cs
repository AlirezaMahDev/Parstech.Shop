...
public class
    ProductRepresentaionPagingQueryHandler : IRequestHandler<ProductRepresentaionPagingQueryReq,
    ProductRepresentationPagingDto>
{
    ...
    public async Task<ProductRepresentationPagingDto> Handle(ProductRepresentaionPagingQueryReq request,
        CancellationToken cancellationToken)
    {
        ...
        {
            categury =
                $"AND EXISTS (SELECT 1 FROM dbo.ProductCategury WHERE ProductId = dbo.ProductStockPrice.ProductId AND CateguryId = {request.ProductRepresenationParameterDto.Categury}) ";
        }
        ...
        int skip = (request.ProductRepresenationParameterDto.CurrentPage - 1) *
                   request.ProductRepresenationParameterDto.TakePage;
        string query =
        ...
        foreach (DapperProductDto item in productReps)
        {
            ...
            if (Pdto.TypeId == 2)
            {
                List<DapperProductDto> variations = DapperHelper.ExecuteCommand(_connectionString,
                    conn => conn.Query<DapperProductDto>(_productQueries.GetListVariationByParentId,
                            new { @rep = request.ProductRepresenationParameterDto.RepId, @parentId = Pdto.ProductId })
                        .ToList());
