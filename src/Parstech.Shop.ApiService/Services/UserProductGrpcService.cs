...
using Shop.Application.Features.UserProduct.Requests.Commands;
...
namespace Shop.ApiService.Services
{
    public class UserProductGrpcService : UserProductService.UserProductServiceBase
    {
        ...
        public override async Task<UserProductResponse> CreateUserProduct(CreateUserProductRequest request, ServerCallContext context)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                {
                    Success = false,
                    Message = $"خطا در افزودن محصول به لیست: {ex.Message}"
                };
            }
        }
        
        public override async Task<UserProductResponse> DeleteUserProduct(DeleteUserProductRequest request, ServerCallContext context)
        {
            try
            {
                ...
                {
                    Success = true,
                    Message = "محصول با موفقیت از لیست شما حذف شد"
                };
            }
            catch (Exception ex)
            {
                ...
                {
                    Success = false,
                    Message = $"خطا در حذف محصول از لیست: {ex.Message}"
                };
            }
        }
        ...
        public override async Task<GetUserProductsResponse> GetUserProducts(GetUserProductsRequest request, ServerCallContext context)
        {
            try
            {
                var products = request.Type.ToLower() switch
                {
                    ...
                    _ => throw new RpcException(new Status(StatusCode.InvalidArgument, "Invalid product type. Use 'Favorite' or 'Compare'."))
                };
