...
public class ProductInventoryAdminGrpcService : ProductInventoryAdminService.ProductInventoryAdminServiceBase
{
    ...
    public override async Task<ProductStockPriceResponse> GetProductStockPrice(ProductStockPriceRequest request,
        ServerCallContext context)
    {
        try
        {
            void stockPrice = await _mediator.Send(new ProductStockPriceReadCommandReq(request.StockPrice.Id));

            return new ProductStockPriceResponse { IsSuccess = true, StockPrice = MapToDto(stockPrice) };
        }
        catch (Exception ex)
        {
            return new ProductStockPriceResponse
            {
                IsSuccess = false, Message = $"Failed to retrieve stock price: {ex.Message}"
            };
        }
    }

    public override async Task<ProductStockPriceListResponse> GetProductStockPrices(ProductIdRequest request,
        ServerCallContext context)
    {
        try
        {
            void stockPrices = await _mediator.Send(new ProductStockPricesReadByProductQueryReq(request.ProductId));

            var response = new ProductStockPriceListResponse { IsSuccess = true };
        ...
        catch (Exception ex)
        {
            ...
            {
                IsSuccess = false, Message = $"Failed to retrieve stock prices: {ex.Message}"
            };
        }
    }

    public override async Task<ResponseDto> CreateProductStockPrice(ProductStockPriceRequest request,
        ServerCallContext context)
    {
        try
        {
            var stockPriceDto = MapFromDto(request.StockPrice);
            void result = await _mediator.Send(new ProductStockPriceCreateCommandReq(stockPriceDto));

            // Refresh parent product quantity
            await _mediator.Send(new RefreshParentQuantityQueryReq(result.Id));

            return new() { IsSuccessed = true, Message = "Stock price created successfully" };
        }
        catch (Exception ex)
        {
            return new() { IsSuccessed = false, Message = $"Failed to create stock price: {ex.Message}" };
        }
    }

    public override async Task<ResponseDto> UpdateProductStockPrice(ProductStockPriceRequest request,
        ServerCallContext context)
    {
        try
        {
            var stockPriceDto = MapFromDto(request.StockPrice);
            void result = await _mediator.Send(new ProductStockPriceUpdateCommandReq(stockPriceDto));

            // Refresh parent product quantity
            await _mediator.Send(new RefreshParentQuantityQueryReq(result.Id));

            return new() { IsSuccessed = true, Message = "Stock price updated successfully" };
        }
        catch (Exception ex)
        {
            return new() { IsSuccessed = false, Message = $"Failed to update stock price: {ex.Message}" };
        }
    }

    public override async Task<ResponseDto> DeleteProductStockPrice(ProductStockPriceIdRequest request,
        ServerCallContext context)
    {
        try
        {
            ...
            return new() { IsSuccessed = true, Message = "Stock price deleted successfully" };
        }
        catch (Exception ex)
        {
            return new() { IsSuccessed = false, Message = $"Failed to delete stock price: {ex.Message}" };
        }
    }

    public override async Task<ResponseDto> QuickEditProductStockPrice(QuickEditRequest request,
        ServerCallContext context)
    {
        try
        {
            // Get current stock price
            void currentStockPrice = await _mediator.Send(new ProductStockPriceReadCommandReq(request.Id));
            ...
            // Save changes
            void edit = await _mediator.Send(new ProductStockPriceUpdateCommandReq(currentStockPrice));
            ...
            return new() { IsSuccessed = true, Message = "Stock price updated successfully via quick edit" };
        }
        catch (Exception ex)
        {
            return new() { IsSuccessed = false, Message = $"Failed to update stock price: {ex.Message}" };
    ...
    public override async Task<ProductStockPriceListResponse> GetProductInventoryByStore(StoreIdRequest request,
        ServerCallContext context)
    {
        try
        {
            void stockPrices = await _mediator.Send(new ProductStockPricesReadByStoreQueryReq(request.StoreId));
        ...
        catch (Exception ex)
        {
            return new ProductStockPriceListResponse
            {
                IsSuccess = false, Message = $"Failed to retrieve store inventory: {ex.Message}"
            };
        }
    }

    public override async Task<StoreInventorySummaryResponse> GetStoreInventorySummary(StoreIdRequest request,
        ServerCallContext context)
    {
        try
        {
            void store = await _mediator.Send(new StoreReadCommandReq(request.StoreId));
            void stockPrices = await _mediator.Send(new ProductStockPricesReadByStoreQueryReq(request.StoreId));
        ...
        catch (Exception ex)
        {
            ...
            {
                IsSuccess = false, Message = $"Failed to retrieve store inventory summary: {ex.Message}"
            };
        }
    }
