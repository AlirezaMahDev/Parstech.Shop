...
namespace Parstech.Shop.ApiService.Services.GrpcServices
{
    public class SelectionsAdminGrpcService : SelectionsAdminService.SelectionsAdminServiceBase
    {
        ...
        public override async Task<SectionListResponse> GetDiscountSectionsSelect(EmptyRequest request, ServerCallContext context)
        {
            try
            {
        ...
        public override async Task<ProductSelectListResponse> GetProductsSelect(EmptyRequest request, ServerCallContext context)
        {
            try
            {
                var products = await _mediator.Send(new ProductSelectListQueryReq());
                
                var response = new ProductSelectListResponse
                {
                    IsSuccess = true
                };
        ...
        public override async Task<CategorySelectListResponse> GetCategoriesSelect(EmptyRequest request, ServerCallContext context)
        {
            try
            {
                var categories = await _mediator.Send(new CategurySelectListQueryReq());
                
                var response = new CategorySelectListResponse
                {
                    IsSuccess = true
                };
