...
namespace Shop.ApiService.Services
{
    public class BrandAdminGrpcService : BrandAdminService.BrandAdminServiceBase
    {
        ...
        public override async Task<BrandPageingDto> GetBrandsForAdmin(BrandParameterRequest request, ServerCallContext context)
        {
        ...
        private Shop.Application.DTOs.Brand.BrandDto MapFromBrandDto(BrandDto brand)
        {
            return new Shop.Application.DTOs.Brand.BrandDto
            {
                ...
                LastChangeTime = !string.IsNullOrEmpty(brand.LastChangeTime) ? 
                    DateTime.Parse(brand.LastChangeTime) : DateTime.Now,
                IsDelete = brand.IsDelete
            };
        }
    }
