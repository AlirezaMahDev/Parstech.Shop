...
using Shop.Application.Features.UserProduct.Commands;
using Shop.Application.Features.UserProduct.Queries;
    ...
    public class UserProductService : UserProductServiceBase
    {
        ...
        public override async Task<UserProductResponse> CreateUserProduct(CreateUserProductRequest request, ServerCallContext context)
        {
            try
            {
                ...
                if (result)
                {
                    var userProduct = await _userProductRepository.GetUserProductByUserAndProduct(request.UserName, request.ProductId);
