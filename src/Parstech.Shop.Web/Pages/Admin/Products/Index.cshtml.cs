...
public class IndexModel : PageModel
{
    ...
    public async Task<IActionResult> OnGet()
    {
        ...
        ProducyTypes = typesResponse.Types.Select(t => new ProductTypeDto { Id = t.Id, Name = t.Name }).ToList();

        var taxesResponse = await _productAdminClient.GetTaxesAsync();
        TaxList = taxesResponse.Taxes.Select(t => new TaxDto
            {
                Id = t.Id, Title = t.Title, Percent = t.Percent, Code = t.Code
            })
            .ToList();
    ...
    public async Task<IActionResult> OnPostEditOrCreateProduct()
    {
        if (ProductDto.Id != 0)
        {
            ...
            var quickEditDto = _mapper.Map<Shop.Shared.Protos.ProductAdmin.ProductQuickEditDto>(productGrpc);
    ...
    private ProductPageingDto MapFromProductPageingDto(Shop.Shared.Protos.ProductAdmin.ProductPageingDto dto)
    {
    ...
    private ProductDto MapFromProductDto(Shop.Shared.Protos.ProductAdmin.ProductDto dto)
    {
    ...
    private Parstech.Shop.Shared.Protos.ProductAdmin.ProductDto MapToProductGrpcDto(ProductDto product)
    {
        ...
        {
            dto.DiscountDate =
                Google.Protobuf.WellKnownTypes.Timestamp.FromDateTime(product.DiscountDate.Value.ToUniversalTime());
        }

        if (product.CreateDate.HasValue)
        {
            dto.CreateDate =
                Google.Protobuf.WellKnownTypes.Timestamp.FromDateTime(product.CreateDate.Value.ToUniversalTime());
        }
    ...
    public async Task<IActionResult> OnPostCateguries()
    {
        ...
        categuries = categoriesResponse.Categories.Select(c => new ProductCateguryDto
            {
                Id = c.Id, ProductId = c.ProductId, CateguryId = c.CategoryId, CateguryName = c.CategoryName
            })
            .ToList();
    ...
    public async Task<JsonResult> OnPostProductStores(int ProductId)
    {
        if (User.IsInRole("Store") || User.IsInRole("StoreBySend"))
        {
            ...
            var item = new ProductStockPriceStoreDto() { Id = 0, StoreName = "دسترسی ندارید" };
            list.Add(item);
            Response.Object = list;
            Response.IsSuccessed = true;
        }
        else
        {
            ...
            var list = response.Stores.Select(s => new ProductStockPriceStoreDto
                {
                    Id = s.Id, ProductId = s.ProductId, StoreId = s.StoreId, StoreName = s.StoreName
                })
                .ToList();
