...
public class ProductAdminGrpcClient : GrpcClientBase, IProductAdminGrpcClient
{
    ...
    public async Task<ProductPageingDto> GetProductsForAdminAsync(int currentPage,
        int takePage,
        string searchKey,
        string filterCat = "",
        string filter = "")
    ...
    {
        var request = new ProductDuplicateForStoreRequest { ProductId = productId, StoreId = storeId };
    ...
    {
        var request = new ProductParentsRequest { Filter = filter ?? string.Empty, Type = type };
