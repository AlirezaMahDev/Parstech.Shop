...
public class FinancialAdminGrpcClient : GrpcClientBase, IFinancialAdminGrpcClient
{
    ...
    {
        var request = new Shop.Shared.Protos.FinancialAdmin.ParameterDto
        {
        ...
        string? itemsString = Encoding.UTF8.GetString(response.Items.ToByteArray());
        string? filterString = Encoding.UTF8.GetString(response.Filter.ToByteArray());
    ...
    {
        var request = new BlockWalletRequest { IsBlocked = isBlocked, WalletId = walletId };
    ...
    {
        var request = new Shop.Shared.Protos.FinancialAdmin.WalletTransactionParameterDto
        {
            PageId = parameter.PageId,
            Take = parameter.Take,
            SearchKey = parameter.SearchKey,
    ...
    {
        var request = new Shop.Shared.Protos.FinancialAdmin.WalletTransactionDto
        {
            Id = transaction.Id,
        ...
        var result = new ResponseDto { Status = response.Status, Message = response.Message, Code = response.Code };
    ...
    {
        var request = new Shop.Shared.Protos.FinancialAdmin.FacilitiesDto
        {
            Id = facilities.Id,
