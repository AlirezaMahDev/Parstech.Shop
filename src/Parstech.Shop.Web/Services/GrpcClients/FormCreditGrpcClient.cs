...
public class FormCreditGrpcClient : GrpcClientBase
{
    ...
    {
        var request = new CreateFormCreditRequest { FormCredit = formCredit };
        ...
        return new ResponseDto
        {
            ...
            Errors = response.Errors.Select(e =>
    ...
    public async Task<List<FormCreditDto>> GetFormCreditsAsync(string filter = "",
        string fromDate = "",
        string toDate = "")
    {
        var request = new GetFormCreditsRequest { Filter = filter, FromDate = fromDate, ToDate = toDate };

        var response = await _client.GetFormCreditsAsync(request);
        return response.FormCredits.ToList();
    }

    public async Task<(List<FormCreditDto> Items, int TotalCount, int PageCount)> GetPagedFormCreditsAsync(int skip,
        int take,
        string filter = "",
        string fromDate = "",
        string toDate = "")
    ...
    {
        var request = new ChangeFormCreditStatusRequest { Id = id, Type = type };

        var response = await _client.ChangeFormCreditStatusAsync(request);

        return new ResponseDto { IsSuccessed = response.IsSuccess, Message = response.Message };
    }
}