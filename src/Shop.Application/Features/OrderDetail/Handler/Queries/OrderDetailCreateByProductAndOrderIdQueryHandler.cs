    ...
    public class OrderDetailCreateByProductAndOrderIdQueryHandler : IRequestHandler<OrderDetailCreateByProductAndOrderIdQueryReq, ResponseDto>
    {
        ...
        public async Task<ResponseDto> Handle(OrderDetailCreateByProductAndOrderIdQueryReq request, CancellationToken cancellationToken)
        {
            ...
            var refreshDetail = _mapper.Map<OrderDetailDto>(orderResult);
             await _mediator.Send(new RefreshOrderDetailQueryReq(refreshDetail,false));
            result.IsSuccessed=true;
            result.Message = "محصول با موفقیت به سبد خرید شما اضافه شد";
            result.Object = item.OrderId;
            return result;
        }
    }

    public class OrderDetailCreateByProductAndOrderIdForCreditProductQueryHandler : IRequestHandler<OrderDetailCreateByProductAndOrderIdForCreditProductQueryReq, ResponseDto>
    {
        private IOrderRepository _orderRep;
        private IUserRepository _userRep;
        private IOrderDetailRepository _orderDetailRep;
        private IProductRepository _productRep;
        private IProductStockPriceRepository _productStockRep;
        private ICreditProductStockPriceReopsitory _creditProductRep;
        private IMapper _mapper;
        private IMediator _mediator;

        public OrderDetailCreateByProductAndOrderIdForCreditProductQueryHandler(IOrderRepository orderRep,
            IOrderDetailRepository orderDetailRep,
            IProductRepository productRep,
            IMapper mapper,
            IMediator mediator,
            IProductStockPriceRepository productStockRep,
            IUserRepository userRep,
            ICreditProductStockPriceReopsitory creditProductRep)
        {
            _orderRep = orderRep;
            _orderDetailRep = orderDetailRep;
            _mapper = mapper;
            ...
            _productStockRep = productStockRep;
            _userRep = userRep;
            _creditProductRep = creditProductRep;
        }

        public async Task<ResponseDto> Handle(OrderDetailCreateByProductAndOrderIdForCreditProductQueryReq request, CancellationToken cancellationToken)
        {
            ResponseDto result = new ResponseDto();
            Shop.Domain.Models.OrderDetail item = new Shop.Domain.Models.OrderDetail();
            var credit =await _creditProductRep.GetAsync(request.creditId);
            var productStock = await _productStockRep.GetAsync(credit.ProductStockPriceId);
            item.ProductStockPriceId = productStock.Id;
            long Price = 0;


            item.Count = 1;
            item.Price = credit.Total;
            item.OrderId = request.orderId;
            item.DetailSum = 0;
            item.Tax = 0;
            item.Discount = 0;
            ...
            var orderResult = await _orderDetailRep.AddAsync(item);
            var refreshDetail = _mapper.Map<OrderDetailDto>(orderResult);
            await _mediator.Send(new RefreshOrderDetailQueryReq(refreshDetail,true));
            result.IsSuccessed = true;
            result.Message = "محصول با موفقیت به سبد خرید شما اضافه شد";
            result.Object = item.OrderId;
            return result;
