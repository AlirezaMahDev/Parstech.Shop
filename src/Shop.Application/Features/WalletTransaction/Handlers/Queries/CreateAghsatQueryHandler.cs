...
namespace Shop.Application.Features.WalletTransaction.Handlers.Queries
{
    ...
    public class CreateAghsatForCreditProductQueryHandler : IRequestHandler<CreateAghsatForCreditProductQueryReq, bool>
    {
        private IWalletTransactionRepository _walletTransactionRep;
        private IWalletRepository _walletRep;
        private IMediator _mediator;

        public CreateAghsatForCreditProductQueryHandler(IWalletTransactionRepository walletTransactionRep, IMediator mediator, IWalletRepository walletRep)
        {
            _walletTransactionRep = walletTransactionRep;
            _mediator = mediator;
            _walletRep = walletRep;
        }
        public async Task<bool> Handle(CreateAghsatForCreditProductQueryReq request, CancellationToken cancellationToken)
        {
            //var transaction = await _walletTransactionRep.GetAsync(request.transactionId);
            var wallet = await _walletRep.GetWalletByUserId(request.order.UserId);


            FecilitiesDto req = new FecilitiesDto();
            req.Price = request.credit.PayMonth;
            req.Ghest = request.credit.PayMonth;
            req.Sud = request.credit.Persent;
            req.GhestCount = request.credit.Month;
            req.Karmozd = 0;
            req.WalletId = wallet.WalletId;
            req.Type = "ProductCredit";
            req.OrderCode = request.order.OrderCode;
            req.ParentFecilitiesId = null;

            
            await _walletTransactionRep.CreateNewFesilities(req);
