                ...
                foreach (var item in res.orderPayResult)
                {
                    switch (item.PayTypeId)
                    {
                        ...
                        case 7:
                            ...
                            Response.Object = await _mediator.Send(new DargaQueryReq(order.OrderCode, createdTransaction7.walletTransaction.Id, item.Price));
                        ...
                        //پرداخت اعتباری بدون پیش پرداخت
                        case 8:

                            //await _mediator.Send(new CreateAghsatForCreditProductQueryReq(order,));
